...
- 所有新增或修改的界面必须在同一次实现中同时兼容浅色与暗色模式；除非用户明确要求固定颜色，否则不得把暗色模式适配留作后续工作。
- 普通背景、卡片、按钮、输入框、弹窗、边框和文字必须优先使用 `AppTheme` 中基于 macOS 系统颜色定义的语义色。不得使用固定的 `Color.white`、`Color.black` 或浅灰 RGB 值充当普通界面背景或前景。
- hover、pressed、focused、selected、disabled 等交互状态必须继承同一套语义色规则，不能只适配默认状态。修改某一状态时，要静态检查其他状态在两种外观下是否仍有足够对比度。
- 品牌色、警示色或为保证特定彩色背景对比度而使用的固定颜色属于例外；使用时应让固定前景色与对应背景成对出现，避免单独复用到会随系统外观变化的表面上。
